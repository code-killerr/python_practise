...
def printcolor(req):
    ...
    uri = 'data:image/png;base64,' + urllib.parse.quote(base64.b64encode(buf.getbuffer()))
    context = '<img src = "'+uri+'"/>'
...
def popu(req):    
    ...
    m.readshapefile('E:\python\python\python_practise\Django\HelloWorld\HelloWorld\gadm36_CHN_shp/gadm36_CHN_1', 'states', drawbounds=True)
    m.readshapefile('E:\python\python\python_practise\Django\HelloWorld\HelloWorld\gadm36_TWN_shp/gadm36_TWN_1', 'taiwan', drawbounds=True)
    ax = plt.gca()
    for nshape, seg in enumerate(m.taiwan):
        poly = Polygon(seg, facecolor="orange")
        ax.add_patch(poly)
    df = pd.read_csv('E:\python\python\python_practise\Django\HelloWorld\HelloWorld\A22a.csv')
    new_index_list = []
    for i in df["地区"]:
        i = i.replace(" ","")
    ...
    html_graph = mpld3.fig_to_html(fig)
    plt.close()
    return render("popu.html",({"graph":html_graph}))
    '''
    buf = BytesIO()
    fig.savefig(buf, format='png')
    buf.seek(0)  # rewind the data
